...
int main(int argc, char* argv[])
{
    ...
    //代表服务器跟客户端的TCP连接
    int newFdSet[3];
    bzero(newFdSet,sizeof(newFdSet));
    int newFd;//表示新客户端描述符

    int clientNum=0;//记录客户端数量
    ...
    //因为rdset经过select会发生改变，select会把就绪的描述符保存在rdset中
    //所以用needMonitorSet去保存监听的描述符
        ...
        if(readyNum > 0)
        {
            ...
            if(FD_ISSET(STDIN_FILENO,&rdset))
            {
                ...
                for(int i=0;i<clientNum;i++){
                    if(FD_ISSET(newFdSet[i],&rdset))
                    {
                        bzero(buf,sizeof(buf));
                        //客户端断开的时候，内核会把newFd标记为可读
                        //select就会一直检测到newFd是可读的
                        //就会一直进入到这个分支
                        ret = recv(newFdSet[i],buf,sizeof(buf),0);
                        //对端断开的退出机制
                        //当对方断开，recv的返回值为0
                        if(ret == 0)
                        {
                            ...
                            printf("byebye\n");
                            FD_CLR(newFdSet[i],&needMonitorSet);
                            //将newFd送集合中删掉
                            //关闭连接
                            close(newFdSet[i]);
                        }
